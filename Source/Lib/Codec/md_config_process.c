...
void *svt_aom_mode_decision_configuration_kernel(void *input_ptr) {
    ...
    for (;;) {
        ...
        if ((skip_perc > 75 && cdef_ctrls->use_skip_detector) ||
            (scs->vq_ctrls.sharpness_ctrls.cdef && pcs->ppcs->is_noise_level && pcs->scs->static_config.noise_detect))
        ...
        if (scs->vq_ctrls.sharpness_ctrls.restoration && pcs->ppcs->is_noise_level && pcs->scs->static_config.noise_detect) {
