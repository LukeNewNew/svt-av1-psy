    ...
    PictureControlSet *pcs)
{
    ...
    int8_t noisenorm_strength = pcs->scs->static_config.noise_norm_strength;

    // If tune is 3 & noisenorm_strength is not set, set noisenorm_strength to 3
    if (noisenorm_strength == DEFAULT && tune == 3) {
        noisenorm_strength = 3;
    } else if (noisenorm_strength == DEFAULT) {
        noisenorm_strength = 0;
    }
